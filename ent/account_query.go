...
func (aq *AccountQuery) QueryFromTransfers() *TransferQuery {
...
func (aq *AccountQuery) QueryToTransfers() *TransferQuery {
	query := &TransferQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (aq *AccountQuery) WithFromTransfers(opts ...func(*TransferQuery)) *AccountQuery {
	query := &TransferQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withFromTransfers = query
	return aq
}
...
func (aq *AccountQuery) WithToTransfers(opts ...func(*TransferQuery)) *AccountQuery {
	query := &TransferQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AccountQuery) sqlAll(ctx context.Context) ([]*Account, error) {
	...
	if query := aq.withEntries; query != nil {
		...
		for _, n := range neighbors {
			...
			node, ok := nodeids[fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "account_id" returned %v for node %v`, fk, n.ID)
			}
			node.Edges.Entries = append(node.Edges.Entries, n)
		}
	}

	if query := aq.withFromTransfers; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Account)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
	...
	if query := aq.withToTransfers; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Account)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
