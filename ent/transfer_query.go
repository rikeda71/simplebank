...
type TransferQuery struct {
	...
	// eager-loading edges.
	withFromAccounts *AccountQuery
	withToAccounts   *AccountQuery
...
// QueryFromAccounts chains the current query on the "from_accounts" edge.
func (tq *TransferQuery) QueryFromAccounts() *AccountQuery {
...
// QueryToAccounts chains the current query on the "to_accounts" edge.
func (tq *TransferQuery) QueryToAccounts() *AccountQuery {
	query := &AccountQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (tq *TransferQuery) Clone() *TransferQuery {
	...
	return &TransferQuery{
		...
		withFromAccounts: tq.withFromAccounts.Clone(),
		withToAccounts:   tq.withToAccounts.Clone(),
...
// WithFromAccounts tells the query-builder to eager-load the nodes that are connected to
// the "from_accounts" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TransferQuery) WithFromAccounts(opts ...func(*AccountQuery)) *TransferQuery {
	query := &AccountQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withFromAccounts = query
	return tq
}

// WithToAccounts tells the query-builder to eager-load the nodes that are connected to
// the "to_accounts" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TransferQuery) WithToAccounts(opts ...func(*AccountQuery)) *TransferQuery {
	query := &AccountQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withToAccounts = query
	return tq
}
...
func (tq *TransferQuery) sqlAll(ctx context.Context) ([]*Transfer, error) {
	var (
		...
		_spec       = tq.querySpec()
		loadedTypes = [2]bool{
			tq.withFromAccounts != nil,
			tq.withToAccounts != nil,
		}
	)
	...
	if query := tq.withFromAccounts; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Transfer)
		for i := range nodes {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "from_account_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.FromAccounts = n
			}
		}
	}

	if query := tq.withToAccounts; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Transfer)
		for i := range nodes {
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "to_account_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.ToAccounts = n
			}
		}
	}
