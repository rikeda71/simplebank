...
func (eq *EntryQuery) QueryAccounts() *AccountQuery {
...
func (eq *EntryQuery) WithAccounts(opts ...func(*AccountQuery)) *EntryQuery {
	query := &AccountQuery{config: eq.config}
...
func (eq *EntryQuery) sqlAll(ctx context.Context) ([]*Entry, error) {
	...
	if query := eq.withAccounts; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "account_id" returned %v`, n.ID)
			}
			for i := range nodes {
...
func (eq *EntryQuery) ForUpdate(opts ...sql.LockOption) *EntryQuery {
...
func (eq *EntryQuery) ForShare(opts ...sql.LockOption) *EntryQuery {
	if eq.driver.Dialect() == dialect.Postgres {
		eq.Unique(false)
	}
	eq.modifiers = append(eq.modifiers, func(s *sql.Selector) {
